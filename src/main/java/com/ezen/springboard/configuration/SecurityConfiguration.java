...
public class SecurityConfiguration {
	...
	// 복호화는 불가능, match(사용자 입력값(그냥 String), DB에 저장된 암호화된 비밀번호) => true 또는 false로 리턴
	@Bean
	...
	public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		// 권한에 따른 요청주소 매핑      // "/home"으로 시작하는 요청 리소스는 모든 사람에게 허용
		...
		http.formLogin()
			...
			// Spring Security에서는 id는 username, 비밀번호는 password로 사용
			...
			.loginPage("/user/login")
			// 토큰 발행 후 처리
			// 토큰이 발행되면 사용자 정보를 받아서 처리 가능해지는데 사용자 정보를 웹 사이트에 맞도록 변경해주는 작업 필요
		...
		// 크로스도메인 이슈 방지 설정
		// 크로스도메인: 웹 생태계는 원칙적으로 하나의 웹 어플리케이션에서 요청에 대한 주소가 변경이 되면 안된다.
		...
		//				타임아웃시간, 인증에 대한 요청, 요청 헤더의 허용여부 등을 설정
		http.csrf().disable();
